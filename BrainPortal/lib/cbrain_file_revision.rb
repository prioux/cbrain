...
class CbrainFileRevision
  ...
  def get_git_rev_info_from_git #:nodoc:
    ...
    #Dir.chdir(dirname) do
      IO.popen("cd #{dirname.to_s.bash_escape} ; git rev-list --max-count=1 --date=iso --pretty=format:'%H %ad %an' HEAD -- ./#{@basename.to_s.bash_escape} 2>/dev/null","r") do |fh|
