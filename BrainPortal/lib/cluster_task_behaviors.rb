...
#Abstract module representing a job running on a cluster. This is the core class for
...
module ClusterTaskBehaviors
  ...
  def make_available(userfile, file_path, userfile_sub_path = nil, start_dir = nil)
    ...
    unless start_dir
      FileUtils.mkpath(full_path.dirname) unless Dir.exist?(full_path.dirname)
  ...
  def ready_to_post_process?
    ...
    return true  if     File.exist?(stdout_file) && File.size(stdout_file) > 0
  ...
  def find_or_create_combined_file(qsub_outerr, science_outerr) #:nodoc:
    ...
    return combined_file  if File.exist?(combined_file)

    # Task outputs incomplete? Point to science output captured
    return science_outerr if ! File.exist?(qsub_outerr)
  ...
  def extract_cpu_times_from_qsub_wrapper(filename)
    return nil unless File.exist?(filename.to_s)
...
#{bourreau_glob_config ? bourreau_glob_config.to_bash_epilogue : ""}
    ...
    if File.exist?(rt_info_script) && ! File.exist?(".runtime_info.sh")
 ...
 # Load the singularity image either from a repository or from a CBRAIN file
  ...
  def load_singularity_image_from_repo #:nodoc:
    ...
    scratch_userfile = SingularityImage.find_or_create_as_scratch(:name => scratch_name) do |cache_path|
      ...
      next if File.exist?(cache_path.to_s) && File.size(cache_path.to_s) > 0
      ...
      if ! File.exist?(cache_path.to_s) || File.size(cache_path.to_s) < 500.kilobytes
